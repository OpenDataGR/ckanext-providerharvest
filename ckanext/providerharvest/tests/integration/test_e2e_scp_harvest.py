"""End-to-end SCP/bulk_file harvest-job scenario against the docker/
replica's real ``scp-provider`` container (a plain OpenSSH server built
specifically for this -- see docker/scp-provider/Dockerfile for why
atmoz/sftp, used for the SFTP test server, can't be reused here).
Phase 1.5's SCP counterpart to test_e2e_sftp_harvest.py: exercises
ScpTransport's real `find`-over-exec-channel listing and real `scp`
protocol file transfer against a real server, not fakes (those already
live in test_transport_scp.py), plus the same
provider_source_fetch_host_key -> provider_source_create ->
provider_source_activate -> gather/import_stage -> FileResourceLoader
chain SFTP's test exercises -- confirming scp/sftp genuinely share that
whole pipeline via SSH_TRANSPORT_TYPES, not just superficially.
...
SCP_ENDPOINT_URL = "sftp://scp-provider"  # scheme is cosmetic -- _parse_host just needs a host
SCP_USERNAME = "produser"
SCP_PASSWORD = "test-pass"


@pytest.fixture(autouse=True)
def _allow_private_network(monkeypatch):
    # Same reasoning as test_e2e_sftp_harvest.py: scp-provider's
...
def _dataset_defaults(name: str):
    return {
        ...
        "notes_translated": {"en": "SCP integration test dataset for %s" % name},
    }
...
class TestFullScpHarvestFlow:
    def test_register_activate_run_and_confirm_resources(self):
        ...
        # Same host-key fingerprint, same trust-on-first-use step SFTP
        # uses -- fetch_host_key_fingerprint is shared SSH connection
        # code (transport/ssh_common.py), not per-subsystem.
        key_info = helpers.call_action(
            "provider_source_fetch_host_key", editor_ctx,
            owner_org=org["id"], endpoint_url=SCP_ENDPOINT_URL,
        )
        ...
        created = helpers.call_action(
            ...
            editor_ctx,
            name="scp-mock-source",
            owner_org=org["id"],
            endpoint_url=SCP_ENDPOINT_URL,
            transport_type="scp",
            auth_type="ssh_credentials",  # inert for scp -- see base_generic._build_transport
            credential_fields={"username": SCP_USERNAME, "password": SCP_PASSWORD},
            delivery_mode="bulk_file",
            remote_path="/home/produser/upload",
            glob_pattern="*.csv",
            host_key_fingerprint=key_info["fingerprint"],
            frequency="MANUAL",
            dataset_defaults=_dataset_defaults("scp-mock-source"),
        )
    ...
    def test_connect_rejects_a_host_key_that_was_never_pinned(self):
        ...
        with pytest.raises(Exception):
            helpers.call_action(
                ...
                frequency="MANUAL",
                dataset_defaults=_dataset_defaults("scp-unpinned-source"),
            )
